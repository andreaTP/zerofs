...
public class PathServiceTest {
    ...
    public void testPathCreation() {
        ...
        JimfsPath relative =
                service.createRelativePath(service.names(ImmutableList.of("foo", "bar")));
        assertAbout(paths())
                .that(relative)
                .hasRootComponent(null)
                .and()
    ...
    public void testHash_usingDisplayForm() {
        ...
        JimfsPath path1 =
                new JimfsPath(pathService, null, ImmutableList.of(Name.create("FOO", "foo")));
        JimfsPath path2 =
                new JimfsPath(pathService, null, ImmutableList.of(Name.create("FOO", "FOO")));
        JimfsPath path3 =
                new JimfsPath(
        ...
        try {
            FILE_SYSTEM =
                    JimfsFileSystems.newFileSystem(
                            new JimfsFileSystemProvider(),
                            URI.create("jimfs://foo"),
                            Configuration.unix());
        } catch (IOException e) {
